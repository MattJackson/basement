#!/usr/bin/env bash
# mobile-audit.sh — wrapper around mobile-audit.ts.
#
# Same shape as comprehensive-smoke.sh — resolves the TS file and runs
# node from frontend/ so the `playwright` import resolves. See the TS
# header for env vars + safety guarantees (READ-ONLY).

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TS_FILE="$REPO_ROOT/scripts/mobile-audit.ts"
