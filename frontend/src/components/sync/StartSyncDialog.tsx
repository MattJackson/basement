...
export function StartSyncDialog({ open, onOpenChange }: StartSyncDialogProps) {
    ...
    <Dialog open={open} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-[525px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Copy objects from a source bucket to a destination bucket across clusters.
          </DialogDescription>
        </DialogHeader>
        ...
        <form id="sync-form" onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            >
              {createMutation.isPending ? "Creating..." : "Start sync"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
