...
Loop:
1. Run the comprehensive smoke harness + feature smoke + mobile audit against live basement.example.com (with matthew/password)
...
2. **v1.13.1.1** — Live verify skin activation works end-to-end on basement.example.com after Watchtower rolls v1.13.1. If it doesn't actually re-skin the page on activation, dispatch a hotfix worker.
...
8. Triage findings; for each bug:
   ...
   - Dispatch via `opencode run -m example.com/qwen3.5 --dir /Users/mjackson/Developer/basement-ui "$(cat prompts/<file>.md)" > /tmp/<cycle>.log 2>&1 &`
...
You ARE opencode + example.com/qwen3.5. Spawn worker freshmen via shell `&` (your sub-workers don't need harness notifications; you poll git instead).
