...
Structured guide for capturing PNG/WebP images used in README, blog posts, and launch materials. **You do NOT capture these yourself** — operator with access to live deploy at `https://basement.example.com` runs through this list in one sitting.
...
Before capturing, verify these features are live on `https://basement.example.com`:
