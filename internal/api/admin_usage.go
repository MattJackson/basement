...
func (s *Server) getUsageOverviewHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Grant count — retired in ADR-0002 v1.1.0e along with the
	// BucketGrant model. The dashboard's "Grants" totals field is
	// preserved on the wire so frontend types don't churn, but it
	// always reports 0 now. A v1.2 cycle may repurpose this slot for
	// UserRegion count if operators ask for it.
	grantCt := 0
