// Package api: capability gates (ADR-0001, v0.9.0f; ADR-0002 v1.1.0e).
//
// All admin-facing handlers go through requireCapability so the gate
// pattern stays uniform and grep-able:
//
...
//     the check fails.
//
// The legacy v0.9.0f `userGrantDriver` + ErrNoGrant + writeNoGrant
// helpers were retired in ADR-0002 cycle v1.1.0e along with the
// per-bucket BucketGrant model. User-tier object ops now live under
// /api/v1/user/regions/* and authenticate via the region's S3 key
// directly — see internal/api/user_regions.go.
package api
