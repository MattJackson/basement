...
func TestResolveRegionToConnection_Happy(t *testing.T) {
	...
	endpoint := "https://s3.basement.example.com"
...
func TestRegionListBuckets_AuditEvent(t *testing.T) {
	...
	if ev.Resource[len(wantPrefix):] != "s3.example.com" {
		t.Errorf("expected host suffix s3.example.com in resource, got %q", ev.Resource[len(wantPrefix):])
	}
	// Detail: accessKey=...
