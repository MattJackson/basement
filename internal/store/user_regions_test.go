...
func TestUserRegions_CreateGet(t *testing.T) {
	...
	if created.Endpoint != "https://s3.example.com" {
...
func TestUserRegions_NormalizeEndpoint(t *testing.T) {
	for _, tt := range []struct {
		...
		{"http preserved vs https", "http://s3.example.com", "http://s3.example.com", true},
		{"non-default port preserved", "https://s3.example.com:9000", "https://s3.example.com:9000", true},
		{"whitespace trimmed", "  https://s3.example.com  ", "https://s3.example.com", true},
...
func TestUserRegions_GetByUserEndpoint(t *testing.T) {
	...
	got, err := r.GetByUserEndpoint(ctx, in.UserID, "https://s3.example.com")
	if err != nil {
	...
	if _, err := r.GetByUserEndpoint(ctx, "nobody", "https://s3.example.com"); !errors.Is(err, ErrUserRegionNotFound) {
...
func TestUserRegions_ListForUser(t *testing.T) {
	...
	a := sampleRegion()
	a.Endpoint = "https://s3.example.com"
...
func TestUserRegions_AddressingStyle_LegacyOnDiskJSON(t *testing.T) {
	...
	// helper is.
	legacy := `[{"id":"legacy-1","userId":"matthew","alias":"home","endpoint":"https://s3.example.com","region":"garage","accessKeyId":"GK_LEGACY","secretKeyEnc":"AAAA","createdAt":"2026-05-01T00:00:00Z","updatedAt":"2026-05-01T00:00:00Z"}]`
...
func TestUserRegions_Create_Validation(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"endpoint missing scheme", func(in *UserRegion) { in.Endpoint = "s3.example.com" }},
